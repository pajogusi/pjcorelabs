# Domínios — PJCore Labs

## Domínio principal

- Domínio: pjcorelabs.com
- Registrador: Porkbun
- Data de aquisição: 2026-07-05
- Renovação: anual

## Domínio Reino Unido

- Domínio: pjcorelabs.co.uk
- Registrador: Porkbun
- Data de aquisição: 2026-07-05
- Renovação: anual

## Emails configurados

Email forwarding gratuito via Porkbun:

- geral@pjcorelabs.com → pajogusi.uk@gmail.com
- contact@pjcorelabs.com → pajogusi.uk@gmail.com

Estado:

- Ambos os endereços foram testados e funcionam.
- Para já não existe mailbox paga/hosted email ativa.
- O sistema atual é apenas encaminhamento de email.

## Emails reservados para futuro

- hello@pjcorelabs.com
- support@pjcorelabs.com


## Redirecionamento

- pjcorelabs.co.uk redireciona para https://pjcorelabs.com.
- Redirecionamento configurado via Porkbun URL Forwarding.
- Estado confirmado:
  - HTTP: 302 para https://pjcorelabs.com
  - HTTPS: 302 para https://pjcorelabs.com
- DNS do domínio .co.uk aponta para o serviço de forwarding da Porkbun.
